# Harvest Home Guides — Book Cover Design Guide

## Figma Cover Specs

**Layout:** Image on top, text block on solid background below (same structure as Lush Lawns).

**Dimensions:** 1600 × 2560 px (Kindle front cover)

**Text Area Colors:**
- Background: Warm cream/linen `#F5F0E1`
- Series title ("Harvest Home Guides"): Terracotta `#C1582A`
- Book title (e.g. "Texas Vegetable Gardening"): Deep forest green `#2D5A3D`
- Author name ("Evan Maxon"): Muted warm gray `#6B5B4E`

**Fonts:**
- Series title + Book title: **Playfair Display Bold** (Google Fonts, free)
- Author name: **Lato** light or regular (Google Fonts, free)

**Workflow:**
1. Generate illustration via Grok using prompts below (image only, no text)
2. Place illustration in top section of Figma template
3. Add text in bottom section using specs above
4. Export as 1600×2560 JPEG

---

## Grok Image Prompts

**Style notes:** Prompts generate the ILLUSTRATION ONLY. Text is overlaid in Figma. Each prompt asks for open space at top 20% and bottom 30% for text, but since we're using the split layout (image top / text bottom), the full image area can be used.

**Grok Prompt:**
> A warm, sunlit vegetable garden scene in the Texas Hill Country. In the foreground, a rustic wooden raised bed overflowing with ripe red tomatoes, jalapeño peppers, purple okra, and black-eyed peas on the vine. A terracotta pot of basil sits nearby. The midground shows dry golden grass and a single sprawling live oak tree casting dappled shade. Background: rolling limestone hills under a vast blue Texas sky with wispy clouds. Golden hour light from the left. Style: rich, warm, photorealistic with a painterly softness. Aspect ratio 5:8 vertical. The top 20% and bottom 30% should be relatively uncluttered (sky at top, soft ground at bottom) to allow text overlay.

**Key Visual Elements:**
- Tomatoes, jalapeños, okra, black-eyed peas
- Live oak tree, limestone hills
- Vast sky, golden hour warmth
- Rustic raised bed or wooden trough planter

**Regional Accent Color:** Burnt orange (#CC5500) and dusty gold (#C9A84C)

**Text Placement:** Title over the soft lower ground/garden edge. Series banner over open sky at top.

---

## 2. Florida Vegetable Gardening

**Grok Prompt:**
> A lush tropical vegetable garden in Florida. Foreground: a vibrant raised bed with sweet potatoes, cherry tomatoes, Everglades tomatoes, Seminole pumpkins trailing over the edge, and lush collard greens. A few ripe Meyer lemons on a small citrus tree to the side. Midground: a white picket fence draped with passion fruit vine. Background: palm trees silhouetted against a soft pastel sunrise sky in coral and lavender. Warm humid morning light with soft diffusion. Style: bright, saturated, photorealistic with painterly warmth. Vertical 5:8 aspect ratio. Top 20% and bottom 30% kept simple for text overlay.

**Key Visual Elements:**
- Seminole pumpkin, sweet potato, Everglades tomatoes, collard greens
- Citrus tree, passion fruit vine
- Palm trees, tropical sunrise
- White picket fence

**Regional Accent Color:** Coral (#FF6F61) and tropical teal (#008C8C)

**Text Placement:** Title over soft foreground ground area. Series banner over pastel sky.

---

## 3. Southeast Vegetable Gardening

**Grok Prompt:**
> A charming Southern vegetable garden beside a weathered gray farmhouse porch. Foreground: abundant rows of purple hull peas, butter beans, ripe red tomatoes on wooden stakes, bushy collard greens, and golden summer squash. A worn straw hat rests on a fence post. Midground: a white wooden garden gate with climbing scarlet runner beans. Background: misty green rolling hills with mature hardwood trees (magnolia, pecan) under a warm, hazy summer sky. Soft, golden afternoon light. Style: warm, nostalgic, photorealistic with a gentle painterly quality. Vertical 5:8. Top and bottom kept open for text.

**Key Visual Elements:**
- Purple hull peas, collards, tomatoes, butter beans, summer squash
- Farmhouse porch, straw hat, garden gate
- Magnolia/pecan trees, rolling green hills
- Nostalgic Southern charm

**Regional Accent Color:** Magnolia white (#F0EAD6) and peach (#FFCBA4)

**Text Placement:** Title over lower garden rows. Banner over hazy sky.

---

## 4. Midwest Vegetable Gardening

**Grok Prompt:**
> A bountiful Midwestern backyard vegetable garden in peak summer. Foreground: a classic garden plot with tall sweet corn stalks, sprawling zucchini with large yellow blossoms, ripe beefsteak tomatoes on wire cages, green beans on a trellis, and heads of cabbage. A red metal watering can sits on the soil. Midground: a red barn partially visible behind a white fence. Background: flat, expansive farmland stretching to the horizon under a big open sky with puffy cumulus clouds. Warm, direct midday summer sun. Style: bright, wholesome, photorealistic with slight painterly warmth. Vertical 5:8. Open sky at top, soft ground at bottom for text.

**Key Visual Elements:**
- Sweet corn, zucchini, beefsteak tomatoes, green beans, cabbage
- Red barn, white fence, red watering can
- Flat farmland horizon, big sky
- Wholesome, heartland feel

**Regional Accent Color:** Barn red (#7B1818) and cornflower blue (#6495ED)

**Text Placement:** Title over lower garden soil area. Banner over open sky.

---

## 5. Northeast Vegetable Gardening

**Grok Prompt:**
> A picturesque New England vegetable garden in late summer. Foreground: lush raised cedar beds filled with heirloom tomatoes (red, yellow, green striped), sugar snap peas on a trellis, rainbow chard, kale, and plump blueberry bushes alongside. A copper watering can and gardening gloves on the bed edge. Midground: a stone wall covered in moss with a wooden garden gate. Background: mature maple and birch trees with hints of early autumn color (just a few orange leaves), a white church steeple peeking through, under a crisp blue sky. Cool, clear morning light. Style: crisp, elegant, photorealistic with painterly softness. Vertical 5:8. Top and bottom open for text.

**Key Visual Elements:**
- Heirloom tomatoes, snap peas, rainbow chard, kale, blueberries
- Cedar raised beds, stone wall, garden gate
- Maple/birch trees, church steeple
- New England elegance, early-autumn hints

**Regional Accent Color:** Cranberry red (#9B1B30) and slate blue (#4A6670)

**Text Placement:** Title over lower bed area. Banner over sky/treetops.

---

## 6. Pacific Northwest Vegetable Gardening

**Grok Prompt:**
> A lush Pacific Northwest vegetable garden on a misty morning. Foreground: dense raised beds of deep green kale, lettuce varieties, sugar snap peas climbing a cedar trellis, plump garlic bulbs freshly pulled, and bright orange carrots half-emerged from dark rich soil. Rain droplets on the leaves. Midground: a mossy wooden fence with climbing nasturtiums. Background: towering Douglas fir trees fading into soft fog, distant suggestion of a snow-capped mountain (Rainier-like). Cool, diffused overcast light with rays breaking through. Style: moody, lush, photorealistic with atmospheric painterly quality. Vertical 5:8. Top misty and bottom soft for text.

**Key Visual Elements:**
- Kale, lettuce, snap peas, garlic, carrots
- Rain droplets, mossy fence, nasturtiums
- Douglas firs, fog, distant mountain
- Moody PNW atmosphere, rich dark soil

**Regional Accent Color:** Evergreen (#1B4332) and misty lavender (#B8A9C9)

**Text Placement:** Title over dark soil foreground. Banner over misty sky/trees.

---

## 7. Southwest Vegetable Gardening

**Grok Prompt:**
> A vibrant desert vegetable garden in the American Southwest. Foreground: adobe-walled raised bed with chile peppers (red and green Hatch chiles), tomatillos in their papery husks, ripe tomatoes, and squash blossoms. A few prickly pear cactus pads at the edge. Midground: traditional ristras (dried red chile strings) hanging from a weathered wooden post. Background: dramatic red rock mesa formations under a vivid turquoise sky with a few high clouds. Warm, bright desert sun. Style: vibrant, bold, photorealistic with painterly warmth. Vertical 5:8. Sky open at top, foreground soft at bottom for text.

**Key Visual Elements:**
- Hatch chiles, tomatillos, squash blossoms, tomatoes
- Adobe wall, ristras, prickly pear
- Red rock mesas, turquoise sky
- Desert gardening character

**Regional Accent Color:** Turquoise (#40E0D0) and adobe clay (#C2702E)

**Text Placement:** Title over adobe/ground area. Banner over turquoise sky.

---

## 8. Northern California Vegetable Gardening

**Grok Prompt:**
> A bountiful Northern California vegetable garden with Mediterranean flair. Foreground: overflowing wooden crates and beds with artichokes, heirloom tomatoes in multiple colors, fresh basil, fennel bulbs, and Meyer lemons on a small tree. A bottle of olive oil and garlic braid nearby for farm-to-table feel. Midground: grapevine-covered arbor over a garden path. Background: golden rolling hills dotted with live oaks, a vineyard row visible in the distance, under a clear warm sky fading from blue to golden at the horizon. Late afternoon golden light. Style: warm, abundant, farm-to-table aesthetic, photorealistic with painterly glow. Vertical 5:8. Open areas at top and bottom for text.

**Key Visual Elements:**
- Artichokes, heirloom tomatoes, basil, fennel, Meyer lemons
- Grapevine arbor, wooden crates
- Golden hills, live oaks, vineyard suggestion
- Farm-to-table, wine country elegance

**Regional Accent Color:** Wine/burgundy (#722F37) and golden ochre (#CC9544)

**Text Placement:** Title over lower crate/ground area. Banner over golden sky.

---

## 9. Mountain West Vegetable Gardening

**Grok Prompt:**
> A high-altitude vegetable garden in the Mountain West (Colorado/Utah feel). Foreground: sturdy raised beds with cool-weather crops — lettuce, spinach, radishes, beets with crimson stems, and snap peas on a simple wire trellis. A few hardy marigolds for companion planting. The soil is dark and gravelly. Midground: a rustic log fence with a garden tool (hoe) leaning against it. Background: dramatic snow-capped Rocky Mountain peaks under a deep blue high-altitude sky with brilliant white clouds. Crisp, bright mountain sunlight with sharp shadows. Style: bright, crisp, photorealistic with painterly clarity. Vertical 5:8. Sky and ground open for text.

**Key Visual Elements:**
- Lettuce, spinach, radishes, beets, snap peas, marigolds
- Raised beds (short season gardening), log fence
- Rocky Mountain peaks, snow caps
- Crisp high-altitude light, resilient gardening

**Regional Accent Color:** Mountain blue (#3B6BA5) and aspen gold (#E8C547)

**Text Placement:** Title over lower bed/soil area. Banner over mountain sky.

---

## 10. Great Plains Vegetable Gardening

**Grok Prompt:**
> A hardy vegetable garden on the Great Plains under a dramatic open sky. Foreground: a well-tended garden plot with rows of hearty tomatoes staked against the wind, bush beans, onions with tops falling over, winter squash (butternut and acorn) sprawling across straw mulch, and sunflowers towering at the garden edge. A windmill-style garden ornament or old metal windmill visible. Midground: a simple wire and post fence with a prairie grass border. Background: vast, flat prairie extending to a huge open sky taking up the top third, dramatic cumulus clouds lit gold and amber by the setting sun. Warm, wind-swept golden hour light. Style: expansive, resilient, photorealistic with painterly warmth. Vertical 5:8. Big sky at top and soft ground at bottom for text.

**Key Visual Elements:**
- Tomatoes, bush beans, onions, butternut/acorn squash, sunflowers
- Windmill, straw mulch, wire fence
- Vast prairie, enormous sky, dramatic clouds
- Resilience, wind, golden light

**Regional Accent Color:** Wheat gold (#DEB054) and prairie sage (#87A878)

**Text Placement:** Title over lower straw mulch/ground. Banner over dramatic sky.

---

## Series Color Palette Summary

| Element | Color | Hex |
|---|---|---|
| Background/base | Warm cream | #F5F0E1 |
| Series banner | Terracotta | #C1582A |
| Primary text | Deep forest green | #2D5A3D |
| Gold accent | Warm gold | #D4A843 |

### Regional Accents

| Book | Accent 1 | Accent 2 |
|---|---|---|
| Texas | Burnt orange #CC5500 | Dusty gold #C9A84C |
| Florida | Coral #FF6F61 | Tropical teal #008C8C |
| Southeast | Magnolia white #F0EAD6 | Peach #FFCBA4 |
| Midwest | Barn red #7B1818 | Cornflower blue #6495ED |
| Northeast | Cranberry red #9B1B30 | Slate blue #4A6670 |
| Pacific Northwest | Evergreen #1B4332 | Misty lavender #B8A9C9 |
| Southwest | Turquoise #40E0D0 | Adobe clay #C2702E |
| Northern California | Wine #722F37 | Golden ochre #CC9544 |
| Mountain West | Mountain blue #3B6BA5 | Aspen gold #E8C547 |
| Great Plains | Wheat gold #DEB054 | Prairie sage #87A878 |

---

## Implementation Notes

1. **Generate illustration only** — all text (title, author, series name) should be added in Canva, InDesign, or similar. The Grok-generated image is the background/hero art.
2. **Request variations** — generate 3-4 variations per cover and pick the best composition.
3. **Upscale** — Grok outputs may need upscaling to 1600×2560. Use an AI upscaler to hit KDP specs.
4. **Paperback covers** — extend the illustration horizontally for full wrap (front + spine + back). Generate a wider version or use content-aware fill for the back cover.
5. **Thumbnail test** — shrink each cover to 150×240px and confirm the composition reads clearly. The vegetable imagery should be recognizable even tiny.
6. **Consistency pass** — after generating all 10, adjust brightness/saturation/warmth so they look like a matched set on a bookshelf or Amazon series page.
